...
export function DetailsSelector({
  ...
  selectedDetails: Array<IDetail>;
  setSelectedDetails: (details: Array<IDetail>) => void;
}) {
  ...
  return (
    <Combobox
      as="div"
      value={selectedDetails.map((detail: IDetail) => detail.name)}
      onChange={(value) => {
        setSelectedDetails(
          value.map((detail: string) => ({
            name: detail,
            value: '',
          }))
        );
      ...
      multiple
    >
      <Combobox.Label className="block text-sm font-medium text-gray-700">Detalles</Combobox.Label>
      <div className="relative mt-1">
        ...
        {filteredDetails?.length > 0 ? (
          <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {filteredDetails.map((detail: string) => (
              <Combobox.Option
                key={detail}
                value={detail}
                className={({active}) =>
